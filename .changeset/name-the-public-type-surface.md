...
`AtomFamilyAtom`, `AtomFamilySelector`, `GlobalAtomFamily`, `EqualFunc`,
`AtomOnMount`, `AtomOnSet`, `SubscribeFn`, `ScopedStore`, and `ScopeFn` are now
exported from the package root, so a typed wrapper can annotate them directly
instead of re-deriving private shapes with `Parameters`/`ReturnType`.
...
`onMount: () => bootstrap(thisAtom)` shape — are unaffected.
