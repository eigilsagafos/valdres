...
When you'd rather `await` than branch on flags, [fromState](/svelte/fromState) plus Svelte's `{#await}` block works directly on the `T | Promise<T>` value (`fromState(sel).current`, not the `resourceState` box above):
