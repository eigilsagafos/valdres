...
description: Connect a valdres store to the Redux DevTools browser extension
---
...
<div className="callout callout-note">
  <div className="callout-title">Requires the extension</div>
  Install the <a href="https://github.com/reduxjs/redux-devtools">Redux DevTools</a> browser extension. Without it, `connectReduxDevtools` warns once and returns a no-op handle.
</div>
...
const store = useStore()
let handle
onMounted(() => (handle = connectReduxDevtools(store)))
...
import {
    ...
    type ReduxDevtoolsHandle,
} from "@valdres/redux-devtools"

@Component({ selector: "devtools-bridge", template: "" })
export class DevtoolsBridge implements OnDestroy {
    private handle: ReduxDevtoolsHandle = connectReduxDevtools(injectStore())
    ngOnDestroy() {
