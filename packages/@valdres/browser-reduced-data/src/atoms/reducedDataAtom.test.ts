...
import { REDUCED_DATA_MEDIA } from "../lib/reducedDataSource"
import { prefersReducedDataSelector } from "../selectors/prefersReducedDataSelector"
import { reducedDataAtom } from "./reducedDataAtom"

const REDUCE = REDUCED_DATA_MEDIA
...
describe("reducedDataAtom", () => {
    test("reads the current preference without subscribing to anything", () => {
        ...
        expect(app.get(reducedDataAtom)).toBe("reduce")
        expect(app.get(prefersReducedDataSelector)).toBe(true)
    ...
    test("repeated reads reuse one MediaQueryList and never attach", () => {
        ...
        for (let i = 0; i < 10; i++)
            expect(app.get(reducedDataAtom)).toBe("no-preference")
        expect(app.get(prefersReducedDataSelector)).toBe(false)
    ...
    test("a direct subscriber attaches exactly one listener and sees changes", () => {
        ...
        const unsub = app.sub(reducedDataAtom, () => seen.push(app.get(reducedDataAtom)))
    ...
    test("a transitive selector subscriber retains the source", () => {
        ...
        const unsub = app.sub(prefersReducedDataSelector, () => seen.push(app.get(prefersReducedDataSelector)))
        expect(media.listeners(REDUCE)).toBe(1)
    ...
    test("an unchanged event settles without notifying selector subscribers", () => {
        ...
        const unsub = app.sub(prefersReducedDataSelector, () => notifications++)
    ...
    test("two independent stores each own a physical listener", () => {
        ...
        const stopFirst = first.sub(reducedDataAtom, () => seen.push("first"))
        const stopSecond = second.sub(reducedDataAtom, () => seen.push("second"))
        expect(media.listeners(REDUCE)).toBe(2)

        media.set(REDUCE, true)
        media.change(REDUCE)
        expect(seen.sort()).toEqual(["first", "second"])
        expect(first.get(reducedDataAtom)).toBe("reduce")
        expect(second.get(reducedDataAtom)).toBe("reduce")
        ...
        expect(second.get(reducedDataAtom)).toBe("no-preference")
    ...
    test("a child scope shares the tree projection instead of attaching again", () => {
        ...
        const stopRoot = app.sub(reducedDataAtom, () => {})
        expect(media.listeners(REDUCE)).toBe(1)

        const stopChild = child.sub(reducedDataAtom, () => {})
        expect(media.listeners(REDUCE)).toBe(1)
        expect(child.get(reducedDataAtom)).toBe("reduce")

        media.set(REDUCE, false)
        media.change(REDUCE)
        expect(child.get(reducedDataAtom)).toBe("no-preference")
    ...
    test("detach then reattach re-samples the preference that changed while dormant", () => {
        ...
        const first = app.sub(reducedDataAtom, () => {})
        expect(media.listeners(REDUCE)).toBe(1)
        first()
        expect(media.listeners(REDUCE)).toBe(0)
        ...
        const second = app.sub(reducedDataAtom, () => {})
        expect(media.listeners(REDUCE)).toBe(1)
        expect(app.get(reducedDataAtom)).toBe("reduce")
    ...
    test("each store owns an independent listener, so one throwing subscriber cannot starve another", () => {
        ...
        const stopFailing = failing.sub(reducedDataAtom, () => {
            throw new Error("subscriber exploded")
        })
        const stopHealthy = healthy.sub(reducedDataAtom, () => {
            healthyRuns++
        })
        ...
        expect(healthy.get(reducedDataAtom)).toBe("reduce")
        expect(failing.get(reducedDataAtom)).toBe("reduce")
