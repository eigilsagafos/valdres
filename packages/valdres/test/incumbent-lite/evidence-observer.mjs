// Incumbent-lite build observation, derived from the frozen common-counter
// translation for the unchanged hosts. Bundled ONLY in the counter artifact.
...
const specificKeys = [
    ...
    "canonicalPositivePaths",
    "transientPrefixExpansions",
    "prefixRevalidationCalls",
    "exactDeltaBatches",
    "exactDeltaEdges",
    "canonicalPrefixFallbacks",
    "prefixSearches",
    "newEdgeSearches",
    "topologyDeltaSearches",
    "activeCycleRejections",
    "memoConsultations",
    "reverseProofSearches",
    "transientReverseSnapshots",
]
let specific
let identities,
    symbols,
...
export const observer = {
    ...
    reset({ trace = true } = {}) {
        ...
        specific = Object.fromEntries(specificKeys.map(k => [k, 0]))
        next = 1
        tracing = trace
    },
    resetCounters({ trace = false } = {}) {
        counts = Object.fromEntries(keys.map(key => [key, 0]))
        specific = Object.fromEntries(specificKeys.map(key => [key, 0]))
        events = []
        tracing = trace
    },
    label(value, label) {
    ...
    count(key, amount = 1) {
        if (!Object.hasOwn(specific, key))
            throw Error("Unknown incumbent counter")
        specific[key] += amount
    },
    search(site) {
        this.count("canonicalSearches")
        this.count(
            ["prefixSearches", "newEdgeSearches", "topologyDeltaSearches"][
                site
            ],
        )
    },
