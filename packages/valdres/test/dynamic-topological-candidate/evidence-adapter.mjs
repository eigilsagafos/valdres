...
// Exact candidate revision-1 anchors. Observation never drives graph decisions.
export function instrumentDynamicTopo(
    source,
    filename,
    ...
    function replace(before, after, count = 1) {
        ...
        if (found !== count)
            throw new Error(
                `DYNAMIC-TOPO-ADAPTER-ANCHOR: ${filename}: expected ${count}, found ${found}: ${before}`,
            )
        source = source.split(before).join(after)
        changed = true
    }
    if (filename.endsWith("/dynamic-topological/order.ts")) {
        for (const [anchor, key] of [
            ["    begin(node: object): void {", "provisionalBegins"],
            [
                "    insert(parent: object, dependency: object): readonly object[] | undefined {",
                "insertions",
            ],
            ["        if (start.rank <= target.rank) {", "rankChecks"],
            ["                const node = stack.pop()!", "forwardNodes"],
            ["                    const next = edges[i]!", "edgeProbes"],
            ["            const low = start.rank", "relabels"],
            ["                this.order[rank] = ref", "rankWrites"],
            ["                    return [parent, ...path]", "rejections"],
            ["    accept(node: object): void {", "acceptances"],
            ["    rollback(node: object): void {", "rollbacks"],
            [
                "export function clearTopologicalHost(host: object): void {",
                "hostClears",
            ],
        ]) {
            const prefix = anchor.match(/^ */)[0]
            if (anchor.endsWith("{") && key !== "rankChecks")
                replace(
                    anchor,
                    anchor + `\n${prefix}    tournamentObserver.topo("${key}")`,
                )
            else
                replace(
                    anchor,
    ...
    } else if (filename.endsWith("/dynamic-topological/evaluate.ts")) {
        replace(
...
export function createEvidencePlugin() {
    return {
        name: "dynamic-topological-tournament-observation",
        setup(build) {
            build.onLoad(
                {
                    filter: /\/(?:dynamic-topological\/(?:evaluate|order)|committed-store-tree\/(?:scope-node|scratch-selector-host|committed-store-tree))\.ts$/,
                },
                args => ({
                    contents: instrumentDynamicTopo(
