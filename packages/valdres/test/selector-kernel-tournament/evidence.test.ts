...
import {
    ...
    same,
    strictKeys,
} from "../../../../scripts/selector-kernel-tournament/evidence.mjs"
...
} from "../../../../scripts/selector-kernel-tournament/provenance.mjs"
import { validateProcess } from "../../../../scripts/selector-kernel-tournament/process-evidence.mjs"
import {
    computeSelection,
    validateControlShape,
} from "../../../../scripts/selector-kernel-tournament/report.mjs"
import {
...
test("declared intent is immutable and removed workload IDs cannot be declared", () =>
    temporary(root => {
        ...
        writeFileSync(
            ...
            JSON.stringify({ ...plan, intendedWorkloads: ["P-FANOUT-512"] }),
        )
...
test("exact invocation, runtime environment, PID and full UTC process evidence are required", () => {
    ...
    expect(() =>
        validateProcess(
            { ...good, argv: ["node", "--import", "cheat.mjs", "worker.mjs"] },
            { argv: good.argv },
        ),
...
test("recorded gates alone determine advancement; reviewer or human enthusiasm cannot rescue a failure", () => {
    ...
    expect(() => computeSelection(gates, "A", { decision: "promote" })).toThrow(
...
test("raw size parsing retains each metric and rejects rehashed baseline substitutions", async () => {
    ...
    expect(() => parseSizeOutput(text + "\n  dist total raw 1 gzip 1")).toThrow(
        "SIZE-METRICS",
    )
})
