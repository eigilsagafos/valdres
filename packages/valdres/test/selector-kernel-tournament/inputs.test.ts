    ...
    describe("F0 frozen authority", () => {
        test("v3 scope cannot regress to legacy async/global or synthetic hydration", () => {
            ...
            for (const [collection, id] of [
                ...
                target[0].id = id
                expect(() => validateInventoryRows(report)).toThrow(
                    "unknown row",
                )
            }
            const changed = copy()
        ...
        test.each(["id", "parameter", "duplicate", "unknown-field"])(
            ...
            mutation => {
                ...
                expect(() => schemaCheck(data, "manifest")).toThrow(
                    "INPUT-SCHEMA",
                )
            },
        )
        ...
        test("memory source ceiling drift fails", () => {
            const source = frozenInputBytes(
                manifest.memoryScenarios[0].sourceTest,
            )
                .toString("utf8")
                .replace(
                    "retainedBytesPerUnit: 160",
                    "retainedBytesPerUnit: 161",
                )
            expect(() => verifyMemorySource(source)).toThrow(
        ...
        test.each(["unknown", "duplicate", "family", "memory", "pre28"])(
            ...
            mutation => {
                ...
                if (mutation === "duplicate")
                    data.workloads.push(data.workloads[0])
                if (mutation === "family") {
        ...
        test("report schema closes fields and candidate IDs", () =>
            expect(() => schemaCheck({ unknown: true })).toThrow(
                "INPUT-SCHEMA",
            ))
    })
}
