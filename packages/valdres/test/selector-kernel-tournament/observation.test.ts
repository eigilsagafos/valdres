...
if (isolatedTournamentFile()) {
    ...
    const { controlSourceBytes } = await import(
    ...
    test("control observation anchors are exhaustive and reject source drift", () => {
        for (const relative of [
            ...
            expect(instrumentControl(source, path)).toContain(
                "tournamentObserver",
            )
    ...
    test("record observation counts set deltas, equal publications and lifecycle clear once", () => {
        ...
        expect(
            result.events.filter(e => e.type === "install")[0].outcome.value
                .kind,
        ).toBe("identity")
    })
    test("within-run identity normalization never traverses application objects", () => {
        ...
        expect(observer.valueToken(NaN)).toEqual({
            kind: "number",
            value: "NaN",
        })
