...
import { normalizeSizes } from "../../../../scripts/selector-kernel-tournament/resource-evidence.mjs"
...
test("historical bytes survive divergent size/StoreTree files; substituted blobs, hashes and candidate paths fail", () => {
    const temporary = mkdtempSync(join(tmpdir(), "tournament-authority-test-")),
    ...
    try {
        ...
        expect(() =>
            instrumentControl(readFileSync(join(root, store), "utf8"), store),
        ...
        expect(controlSourceBytes(store, root).toString("utf8")).toBe(original)
        ...
        for (const row of manifest.frozenInputs) {
            ...
            expect(
                bytes.equals(gitBlob(manifest.control.gitSha, row.path, root)),
            ).toBe(true)
        }
        ...
        expect(
            readFileSync(join(temporary, "evidence/bytes.bin")).equals(bytes),
        ).toBe(true)
    } finally {
