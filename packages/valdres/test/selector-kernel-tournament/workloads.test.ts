...
if (isolatedTournamentFile()) {
    ...
    const { manifest, json, ROOT } = await import(
        "../../../../scripts/selector-kernel-tournament/inputs.mjs"
    )
    const { validateWorkloadExpectations, validateWorkloadSample } =
        await import(
            "../../../../scripts/selector-kernel-tournament/workload-validation.mjs"
        )
    const { buildLegacyWrappers } = await import(
        "../../../../scripts/selector-kernel-tournament/legacy-wrappers.mjs"
    )
