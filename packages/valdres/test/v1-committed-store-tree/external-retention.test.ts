...
test("a dormant projection and reverse routes do not retain abandoned definitions or scopes", async () => {
    ...
    const references = (() => {
        ...
        return [source, ext, selector, child].map(value => new WeakRef(value))
    })()
...
test("stale invalidators and unsubscribe handles retain no departed tree or application objects", async () => {
    let stale!: () => void,
        stop!: () => void,
        cleanups = 0
    const references = (() => {
    ...
    expect(references.every(reference => reference.deref() !== undefined)).toBe(
        true,
    )
    stop()
...
test("disposal severs active ownership even when a source keeps old callbacks forever", async () => {
    const old: (() => void)[] = []
    let cleanups = 0
    const source = {
        getSnapshot: () => 1,
