...
`dispose()` unregisters the store and all of its descendant scopes from only the
global atoms they touched, so cleanup is proportional to work that store did.
Detached scopes are disposed automatically when their last consumer calls
`detach()`. Treat a disposed store as finished; create a new store for later
work.
...
Inside a component you normally read and write through your framework's provider/context instead — see [Provider](/react/Provider).
