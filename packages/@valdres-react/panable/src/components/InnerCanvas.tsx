...
export const InnerCanvas = React.forwardRef<any, { children: React.ReactNode }>(
    ({ children }, ref) => {
        const scale = useScale()
        const { x, y, animate } = useCameraPosition()
        const transform = `scale(${scale}) translate3d(${x}px, ${y}px, 0) translateZ(0)`
        useUpdateAtomRectOnSizeChange(ref, innerCanvasSizeAtom)
