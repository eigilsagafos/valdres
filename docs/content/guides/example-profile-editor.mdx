...
An edit form that uses scoped stores for a "save or cancel" pattern. The edit
form opens in a scope that inherits the current profile — edits stay isolated
until the user saves.

Scoped stores make this pattern unusually low-boilerplate — other libraries can
do it too, but typically require manually cloning the form state and diffing it
back on save.
...
1. User clicks "Edit" — a scoped store is created, inheriting the current
   profile
...
function EditForm({ parentStore, onClose }) {
...
const { parentStore } = defineProps(["parentStore"])
...
function handleSave() {
    ...
    parentStore.set(profileAtom, profile.value)
    emit("close")
}
</script>
...
<template>
    <div>
        ...
        <label
            ...
            <textarea
                :value="profile.bio"
                @input="update('bio', $event.target.value)"
            />
        </label>
...
{#if editing}
  ...
  <div>
    ...
    <button onclick={() => {
        parentStore.set(profileAtom, editProfile.value)
        editing = false
    }}>Save</button>
    ...
    return (
        <div>
            ...
            <label>
                ...
                <textarea
                    value={profile().bio}
                    onInput={e => update("bio", e.target.value)}
                />
            </label>
            <button
                onClick={() => {
                    props.parentStore.set(profileAtom, profile())
                    props.onClose()
                }}
            >
                Save
...
export class ProfileEditForm {
    ...
    handleSave() {
        this.parentStore.set(profileAtom, this.profile())
        this.close.emit()
    }
}
```
...
- **Scoped stores** — edits are isolated in a child store. The parent state is
  untouched until you explicitly save.
- **No copying** — you don't need to manually clone state into local form state.
  The scope inherits everything automatically.
- **Cancel is free** — just unmount the scope. No cleanup, no reset logic.
- **Less boilerplate** — with Jotai, Zustand, or Redux you'd reach the same
  result by manually cloning form state and writing it back; scoped stores
  handle the inherit-and-commit for you.
