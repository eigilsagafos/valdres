...
Valdres owns the transaction lifecycle. The callback receives state operations
such as `get`, `set`, and `reset`, but no `commit()` method or backing store
data. Returning successfully commits once; throwing discards every staged write:
...
// statusAtom was not changed
```

If an operation intentionally needs multiple publish points, use separate
`store.txn()` calls. Each call is an explicit atomic boundary.
...
As with `store.set`, a synchronous transaction write returns its staged value,
while a Promise or promise-returning updater returns a real `Promise`. The
transaction callback itself must remain synchronous—capture the returned Promise
if work outside the transaction needs to await settlement.

```ts
let settlement: Promise<number>

myStore.txn(({ set }) => {
    settlement = set(countA, Promise.resolve(10))
})
