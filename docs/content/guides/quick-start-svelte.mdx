...
writable, so `bind:value` and `count.current++` just work. It also keeps the
updater-form `set` and a `reset`.
...
<button onclick={() => count.set(c => c + 1)}>Increment (updater)</button>
...
`toStore`, to start the subscription.
