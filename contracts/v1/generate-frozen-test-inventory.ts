...
interface Subject {
    ...
    readonly kind: "production-file" | "test-case"
    ...
    const inventory = {
        ...
        provenance: {
            ...
            testRegistration: {
                ...
                assertions: junitSummary.assertions,
...
function captureJunit(): string {
    ...
    try {
        ...
        assert(
            ...
            `node_modules not found: ${nodeModules}`,
        )
        symlinkSync(nodeModules, join(captureDirectory, "node_modules"), "dir")
        run(
            process.execPath,
...
function assertFrozenProvenance(tarballPath: string): void {
    ...
    assert(
        createHash("sha256").update(Uint8Array.from(tarball)).digest("hex") ===
            TARBALL_SHA256,
...
function updateLedgerHeader(source: string): void {
    ...
    header.inventory = {
        ...
        expectedDispositionIds: parsed.entries.map(entry => entry.id),
    }
    header.notes = [
        "The beta.23 production-file and registered test-case inventory is frozen from immutable source trees and the published tarball.",
        "The ledger remains partial until every frozen inventory subject has an approved A/B/C/D/E disposition.",
    ]
    writeFileSync(
        ledgerPath,
...
function checkLedgerHeader(source: string): void {
    ...
    assertExactSet(
        ...
        new Set(inventory.entries.map(entry => entry.id)),
        "ledger expected inventory ID",
    )
}
...
function main(): void {
    ...
    const tarball = resolve(repositoryRoot, tarballArgument ?? DEFAULT_TARBALL)
