...
const requiredFrozenErrorCodes = new Map([
    ...
    ["core.settle-limit-error", "VALDRES_SETTLE_LIMIT"],
...
const storeSubscriberContract = Object.freeze({
    ...
    storeNotes:
        "Store.sub(state, callback: () => void): () => void creates one independent synchronous zero-argument invalidation registration, even when callback identity repeats. Registration performs the same outcome materialization as get, but internally catches the public read throw of a successfully materialized ordinary current error outcome so it can register; only admission, disposal, or internal-publication failure registers nothing. A lifecycle-free registration does not notify. Changed target callback sets enter the frozen post-stability settlement snapshot in first-reaching order; callbacks within each target retain subscription insertion order; delivery is all-fire and each registration runs at most once. Without an already-authoritative post-apply RuntimeMismatchError, the first subscriber throw becomes the SubscriberNotificationError cause and later subscriber throws remain ordered causes. Delivery remains all-fire when that mismatch is already authoritative. If no subscriber throws, the exact RuntimeMismatchError surfaces directly after all callbacks are attempted. If subscriber throws coexist, SubscriberNotificationError is the required outer wrapper with cause equal to the exact RuntimeMismatchError and frozen causes equal to [mismatch, ...subscriber throws in delivery order]; every subscriber throw is retained as a secondary cause in delivery order, and the mismatch remains the semantic primary and is not replaced. The returned unsubscribe is idempotent, removes future eligibility immediately, and cannot edit the current snapshot. Store.sub(state, handlers: { settle?: (transaction) => void; notify?: () => void }): () => void is the phase-keyed form; its settle phase is experimental during the beta. handlers must be an object with at least one of settle and notify and no other own enumerable keys, and each present handler must be a function; each is read once at registration, settle first, and any other value throws TypeError after state validation, registering nothing. If reading a handler disposes the receiving Store, registration throws StoreDisposedError and registers nothing. notify is exactly the callback form and is not a continuation of settle: it runs after its paired settle fails, and admission catch-up can run it without settle. One registration holds both handlers, and its one idempotent unsubscribe removes both. Registration and its admission catch-up never run settle. When a notification boundary changes the target's served token, settle executes as its own synchronous TreeTransaction bound to the registration scope, after selector propagation and before the ordinary frozen notification snapshot is captured. Reads see the latest committed state, including earlier settle commits in the boundary, plus the callback's own staged writes; no triggering value is passed. settle handlers run in waves: first-reaching target order, then registration order; one reached again while still pending reads the newer state, one already run joins the next wave. At most 64 nonempty waves run; if another is needed, pending work is cleared and SettleLimitError is reported. Each settle commit joins the boundary: targets reached across the initial settlement and every settle commit are delivered to ordinary subscribers, including notify handlers, once, deduplicated, with final committed state. The guarantee is per settlement boundary and per StoreTree; it is not cross-tree atomicity or once per DOM event. A settle handler that throws, returns a thenable, or fails staging aborts only its own draft; earlier settle commits and the initiating update remain. A failure after apply keeps that handler's writes and propagates them before delivery. Failures are reported after the one ordinary delivery as SubscriberNotificationError causes ahead of subscriber throws.",
    typeNotes:
